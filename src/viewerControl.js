...
import { imjoyToTfjs, processForShow, inferImgAxesViaSpec } from "./imgProcess";
...
export class ImagejJsController {
  ...
  async getImage() {
    const image = await this.ij.getImage();
    return image;
  }
  ...
  async viewFromUrl(url, inputSpec, outputSpec, type = "input") {
    ...
    if (fileName.endsWith(".npy")) {
      ...
      if (type === "output") {
        const isImg2Img = outputSpec.axes.includes("x") && outputSpec.axes.includes("y");
        if (!isImg2Img) {
